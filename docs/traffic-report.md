...
They are readable directly in Umami. Two more come from the language versions: `lang-switch` (the header toggle, with the language chosen) and `lang-suggest` (the banner offering the other version, `switch` or `dismiss`). Turkish pages live under `/tr/`, and `surfaceOf` maps them to the same surfaces, so a page is counted as one surface whatever its language. The generated guides under `/models/` and `/gpus/` count as the surfaces "Model pages" and "GPU pages"; they carry no query, so they add visits but never a model or device.
